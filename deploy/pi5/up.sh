#!/usr/bin/env bash
# deploy/pi5/up.sh — bring up the pi5 main Adamomaton stack.
#
...
# .env and assumes the images named `adamomaton-<component>:$IMAGE_TAG`
...
# then `docker tag evo-skills-rae:dev adamomaton-skills-rae:$IMAGE_TAG`).
...
# adamomaton-<component>:${IMAGE_TAG} image. Today the operator builds via
# the legacy /thearray/git/evo/Makefile `pi-*` targets and re-tags on the Pi.
echo "    skipped -- images must already exist on ${HOST} as adamomaton-<component>:${IMAGE_TAG}"
